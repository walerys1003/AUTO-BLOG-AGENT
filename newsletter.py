...
def create_newsletter(blog_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if result.get('success'):
                ...
                flash(f'Error creating newsletter: {result.get("error")}', 'danger')
                return redirect(url_for('newsletter.create_newsletter', blog_id=blog_id))
    ...
    # Create suggested title and subject
    suggested_title = f"{blog.name} Weekly Update"
    suggested_subject = f"Latest content from {blog.name}"
...
def send_newsletter(newsletter_id):
    ...
    try:
        # Use the newsletter distributor to send
        distributor = NewsletterDistributor()
        ...
        if result.get('success'):
            ...
            flash(f'Error sending newsletter: {result.get("error")}', 'danger')
...
def api_process_pending():
    ...
    try:
        distributor = NewsletterDistributor()
        result = distributor.process_pending_newsletters()
...
def api_upload_subscribers(blog_id):
    ...
    try:
        blog = Blog.query.get(blog_id)
        if not blog:
            return jsonify({'error': f'Blog with ID {blog_id} not found'})
        
        # Get the distributor
        distributor = NewsletterDistributor()
        
        # Get the email client for the blog
        client = distributor.get_email_client(blog_id)
        
        if not client:
            return jsonify({'error': 'EmailOctopus not configured for this blog'})
        
        # Get all active subscribers for this blog
        subscribers = Subscriber.query.filter_by(status='active')
        if blog_id:
            subscribers = subscribers.filter((Subscriber.blog_id == blog_id) | (Subscriber.blog_id == None))
            
        # Upload subscribers
        result = client.upload_subscribers(subscribers)
